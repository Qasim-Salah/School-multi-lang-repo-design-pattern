            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <div class="col-xs-12">
                        <div class="col-md-12">
                            <br>
                            <form action="{{route('fees.invoices.update',$invoice->id)}}" method="POST" autocomplete="off">
                                @csrf
                                <div class="form-row">
                                    <div class="col">
                                        <label for="student_id" class="mr-sm-2">{{__('students-trans.name')}}</label>
                                        <select class="custom-select" name="student_id" required>
                                            <option value="{{ $invoice->id }}">{{$invoice->student->name }}</option>
                                        </select>
                                        @error('student_id')
                                    ...
                                    <div class="col">
                                        ...
                                        <select class="custom-select" name="fee_id" required>
                                            ...
                                            @foreach($fees as $fee)
                                                <option
                                                    value="{{ $fee->id }}" {{ (old('fee_id',$fee->id) == $invoice->fee_id  ? 'selected':'') }}>{{ $fee->title }}</option>
                                            @endforeach
                                        </select>
                                        @error('fee_id')
                                        <span class="text-danger">{{$message}}</span>
                                ...
                                <div class="form-row">
                                    <div class="col">
                                        ...
                                        <select class="custom-select" name="amount" required>
                                            ...
                                            @foreach($fees as $fee)
                                                <option
                                                    value="{{ $fee->amount }}" {{ (old('amount',$fee->amount) == $invoice->amount  ? 'selected':'') }}>{{ $fee->amount }}</option>
                                            @endforeach
                                        </select>
                                        @error('amount')
                                        <span class="text-danger">{{$message}}</span>
                                ...
                                <div class="form-row">
                                    <div class="col">
                                        ...
                                        <input type="text" class="form-control" name="description" value="{{old('description',$invoice->description)}}" required>
                                    </div>
                                </div>
                                <br>
