...
@endsection
@section('title')
    {{trans('main-trans.students_promotions')}}
@endsection
            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <div class="col-xl-12 mb-30">
                        <div class="card card-statistics h-100">
                            <div class="card-body">
                                <a href="{{route('promotions.create')}}" class="btn btn-success btn-sm" role="button"
                                   aria-pressed="true">{{__('main-trans.add_promotion')}}</a>
                                ...
                                <div class="table-responsive">
                                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                                        ...
                                        <tr>
                                            ...
                                            <th class="alert-info">{{__('students-trans.name')}}</th>
                                        ...
                                        @foreach($promotions as $promotion)
                                            <tr>
                                                ...
                                                <td>{{$promotion->from_grade_r->name}}</td>
                                                <td>{{$promotion->academic_year}}</td>
                                                <td>{{$promotion->from_classroom_r->name }}</td>
                                                <td>{{$promotion->from_section_r->name }}</td>
                                                <td>{{$promotion->from_grade_r->name }}</td>
                                                <td>{{$promotion->academic_year_new}}</td>
                                                <td>{{$promotion->to_classroom_r->name }}</td>
                                                <td>{{$promotion->to_section_r->name }}</td>
                                                <td>
                                                    ...
                                                    <button type="button" class="btn btn-outline-success mb-2"
                                                            data-toggle="modal"
                                                            data-target="#">تخرج الطالب
                                                    </button>
                                                </td>
                                            </tr>
                                        @include('page.students.promotions.delete-all')
                                        @include('page.students.promotions.delete')
                                        @endforeach
                                    </table>
                                </div>
                            </div>
